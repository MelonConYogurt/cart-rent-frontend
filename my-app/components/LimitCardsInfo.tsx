...
export function SingleCardInfoLimited({info}: {info: Datum}) {
...
export function MultipleCarInfoLimited({data}: {data: Datum[]}) {
  const sliceData = data.slice(0, 3);
  return (
    <>
      {sliceData.map((car: Datum) => (
        <SingleCardInfoLimited key={car.vin} info={car} />
      ))}
    </>
  );
}
